...
for (let key in member8) {
  console.log(key);
}
for (let value in member8) {
  console.log(value);
}
for (let key in member8) {
  console.log(member8[key]);
}
// for (let key of member8) { // TypeError: member8 is not iterable
//   console.log(member8[key]);
// }
...
// const user = { name: "mi", age: 47 }; // 같은 객체명으로 객체를 사용하면 배열로 처리 되는게 아니라 ncaught SyntaxError: Identifier 'user' has already been declared
console.log(user);
const user2 = user;
